...
public class BlockDestructor extends BlockContainer {
	 ...
	 */
	public BlockDestructor(boolean active) {
		super(Material.rock);
		...
		if (!active) {
			...
			this.setBlockName("alca259Destructor");
		} else {
			this.setBlockName("alca259DestructorActive");
		}
	}
	...
	public Item getItemDropped(int x, Random yRandom, int z) {
		return Item.getItemFromBlock(Machines.destructor);
	}
	...
	{
		if (this.powered) {
			textureFront = iconReg.registerIcon(Machines.MODID + ":BlockDestructorFrontalActive");
		} else {
			textureFront = iconReg.registerIcon(Machines.MODID + ":BlockDestructorFrontal");
		}
		this.blockIcon = iconReg.registerIcon(Machines.MODID + ":BlockDestructor");
		this.textureSides = iconReg.registerIcon(Machines.MODID + ":BlockDestructor");
	...
	public ItemStack getPickBlock(MovingObjectPosition target, World world, int x, int y, int z) {
        ...
        Block block = Machines.destructor;
	...
	protected boolean tieneDestruccion(int x, int y, int z, String items) {
		LogHelper.debug("[Alca] tieneDestruccion:BlockDestructor.java");
	...
	protected boolean esDestruible(World world, int x, int y, int z) {
		LogHelper.debug("[Alca] esDestruible:BlockDestructor.java");
	...
	public void romperBloque(World world, int x, int y, int z, Random aleatorio) {
		LogHelper.debug("[Alca] romperBloque:BlockDestructor.java");
		// Inicializamos
		int x1 = x;
		int z1 = z;
	...
	public void onBlockPlacedBy(World world, int i, int j, int k, EntityLivingBase entitylivingbase, ItemStack par6ItemStack) {
		LogHelper.debug("[Alca] onBlockPlacedBy:BlockDestructor.java");
	...
	public int onBlockPlaced(World world, int x, int y, int z, int side, float hitX, float hitY, float hitZ, int metadata) {
		LogHelper.debug("[Alca] onBlockPlaced:BlockDestructor.java");
		world.scheduleBlockUpdate(x, y, z, this, this.tickRate(world));
	...
	public void onBlockAdded(World world, int x, int y, int z) {
		LogHelper.debug("[Alca] onBlockAdded:BlockDestructor.java");
        ...
        if (!world.isRemote) {
            ...
            if (this.powered && !world.isBlockIndirectlyGettingPowered(x, y, z)) {
                ...
                world.setBlock(x, y, z, Machines.destructorActive, 0, 2);
	...
	public void updateTick(World world, int x, int y, int z, Random par5Random) {
		LogHelper.debug("[Alca] updateTick:BlockDestructor.java");
		TileEntityPosicion blockEntity = (TileEntityPosicion)world.getTileEntity(x, y, z);
        ...
        if (!world.isRemote) {
        	...
        	if (this.powered && !world.isBlockIndirectlyGettingPowered(x, y, z)) {
                world.setBlock(x, y, z, Machines.destructor, 0, 2);
                world.setBlockMetadataWithNotify(x, y, z, this.direccion, 2);
	...
	public void onNeighborBlockChange(World world, int x, int y, int z, Block block) {
		LogHelper.debug("[Alca] onNeighborBlockChange:BlockDestructor.java");
		TileEntityPosicion blockEntity = (TileEntityPosicion)world.getTileEntity(x, y, z);
